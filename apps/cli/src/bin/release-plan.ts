...
if (import.meta.main) {
	...
	try {
		...
		// A real @actions/core annotation (the outputs go to $GITHUB_OUTPUT, so stdout is free for the
		// workflow command); a hand-written `::warning::` on stderr is never parsed by Actions.
		logWarning(
