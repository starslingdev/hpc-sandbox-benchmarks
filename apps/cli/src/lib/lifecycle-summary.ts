...
/** Round to one decimal so millisecond timings read cleanly without losing sub-ms signal entirely. */
...
export function formatLifecycleLines(summaries: readonly LifecycleMetricSummary[]): string[] {
	return summaries.map(
		(s) => `    ${s.label.padEnd(16)} p50=${s.p50}${s.unit} p95=${s.p95}${s.unit} (n=${s.n})`,
	);
}
