...
test("different suites enter measurement together and retain independent attempt identities", async () => {
	...
	try {
				...
				driver: {
					...
					create: async (...args) => {
						...
						return {
							...session,
							exec: async (command, options) => {
								if (command.includes("benchmark:cpu") || command.includes("benchmark:network")) {
		...
		// The CPU-only fixture cannot fabricate the missing network metrics.
