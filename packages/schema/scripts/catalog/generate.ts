...
export function directionFor(
	...
	if (out instanceof type.errors) {
		throw new Error(
			`profile ${profile.dir}: missing/invalid <Proportion>/<ResultProportion> ("${source ?? "absent"}") for "${description ?? "(wildcard)"}" — declare it in the vendored XML (curation sets dimension/headline/label, never direction)`,
		);
	}
	return out;
}
...
export function generateProfile(profile: PtsProfile): MetricDef[] {
	...
	const { Title, SubTitle, Description } = profile.info;
	...
	const defs = synthesized.map((s): MetricDef => {
		const { description } = s;
		// synthesizeResults already resolved parser-over-profile precedence — no re-derivation here.
		const unit = s.scale;
		if (!unit) {
			throw new Error(
		...
		return {
			...
			dimension,
			unit,
			direction: directionFor(profile, s.proportion, description),
			headline: false,
			label: description
				? ambiguous
	...
	// Zero metrics means the profile silently VANISHES from the catalog — every parser's
	// MatchToTestArguments failed to match any option combination, usually a typo. The parent caught
	// this incidentally via a profile-level direction check; that check is now per-metric, so a
	// zero-metric profile would emit no metric to check and slip through. Keep the failure loud.
	if (defs.length === 0) {
		throw new Error(
			`profile ${profile.dir}: synthesized zero metrics — no <ResultsParser> matched any option combination (check MatchToTestArguments)`,
		);
	}
	return defs;
}
