...
export const catalogSchema = metricDefSchema.array().narrow((cat, ctx) => {
	...
	for (const metric of cat) {
		...
		if (metric.pts.scale !== undefined && metric.unit !== metric.pts.scale) {
			return ctx.mustBe(
				`a pts.scale pin equal to the entry's unit ("${metric.id}" pins "${metric.pts.scale}" but is labeled "${metric.unit}")`,
			);
		}
		if (metric.pts.description === undefined) {
			if (metric.pts.scale !== undefined) {
				return ctx.mustBe(
					`no pts.scale pin on a description-less wildcard ("${metric.id}": the wildcard arm never compares <Scale>, so the pin would be silently ignored)`,
				);
			}
