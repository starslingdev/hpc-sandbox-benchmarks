...
#     and reaps it via EXIT/HUP/INT/TERM traps (so even a watchdog kill of the runner cannot leak
#     the listener into the next trial), and replaces the fixed `sleep 3` with a bounded readiness
#     poll on /proc/net/tcp{,6} (the deleted vendored network-loopback runner's proven pattern)
#     that falls through after ~10s and lets the client fail honestly.
...
#     runs.
#   * Both status files are written on EVERY exit path, not just the happy one. PTS decides an
#     install succeeded by reading ~/install-exit-status and a trial by reading ~/test-exit-status;
#     it does not consult the process exit code, and lib/bench.sh already documents that "PTS can
#     exit 0 even when compilation failed" (the reason run_pts_benchmark re-verifies through
#     list-installed-tests). `set -eu` alone therefore turns a tar/configure/make failure into a
#     MISSING status file rather than a failing one. Upstream leaves both gaps; the repo's other
#     profiles (local/hardlink, fast-cli, lib/pts/realworld) already write an explicit nonzero
#     status on their failure branches, so match that.

# Report the real status to PTS if anything below dies under `set -e`. Guarded on non-zero so the
# happy path's explicit `echo 0` at the end stays the single source of the success value.
...
# Seed a failure status BEFORE anything can go wrong. PTS reads ~/test-exit-status after every trial
# and the file persists between them, so a trial that dies before the write at the end — a watchdog
# TERM, or a server that never comes up — would otherwise leave the PREVIOUS trial's 0 in place and
# be banked as a success.
echo 1 > ~/test-exit-status
"$bin/iperf3" -s -1 >/dev/null 2>&1 </dev/null &
IPERF_SERVER_PID=$!
# Reap the one-off server on EVERY exit path: a watchdog kill of this runner must not leave the
# unserved listener behind, where a later trial's readiness poll could mistake it for its own.
cleanup() {
	kill "$IPERF_SERVER_PID" 2>/dev/null || true
	wait "$IPERF_SERVER_PID" 2>/dev/null || true
}
trap cleanup EXIT
# RECORD the interruption as well as re-exiting with the conventional 128+n code: PTS reads the
# status file, not the process exit code, so the write is what actually makes an interrupted trial
# visible to it. (The seed above already covers a signal that arrives before these are installed.)
trap 'echo 129 > ~/test-exit-status; exit 129' HUP
trap 'echo 130 > ~/test-exit-status; exit 130' INT
trap 'echo 143 > ~/test-exit-status; exit 143' TERM
...
# (Fractional sleep is GNU coreutils, not POSIX — fine here: the /proc/net/tcp poll already makes
# this runner Linux-only.)
attempt=0
