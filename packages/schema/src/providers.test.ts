...
describe("@sandbox-benchmarks/schema providers", () => {
	...
	it("prices shipped providers from their vetted per-second rates", () => {
		...
		const expected: Record<string, number> = {
			...
			vercel: 0.128 * TARGET_SPEC.vcpus + 0.0212 * TARGET_SPEC.memoryGb,
			cloudrun: 0.0648 * TARGET_SPEC.vcpus + 0.0072 * TARGET_SPEC.memoryGb,
	...
	it("records disk rates where published and omits them where not", () => {
		...
		expect(diskRate("vercel")).toBeUndefined(); // no published disk rate
		expect(diskRate("cloudrun")).toBeUndefined(); // in-instance fs only; no published disk rate
	});
