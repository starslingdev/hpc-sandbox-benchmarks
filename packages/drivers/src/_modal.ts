...
export function createModalControlRunner(
	...
	const deadlineMiddleware: ClientMiddleware = async function* (call, options) {
		...
		const nextOptions = {
			...
			signal,
			timeoutMs,
			// Modal 0.9's retry middleware drops `signal` entirely when retries is zero.
			// One bounded retry keeps cancellation attached to the real gRPC transport; the
...
function modalSpec<P extends ModalProviderId>(
	...
	return computeSdkSpec(
		...
		}),
		{
			...
			commands: {
				exec: (sandbox, command, ref) => execModalCommand(runner, sandbox, command, ref),
				launch: (sandbox, command, ref) => launchModalCommand(runner, sandbox, command, ref),
			},
			lifecycle: modalLifecycle(variant, runner),
			createRecovery: modalCreateRecovery(variant, runner),
			verifyCreatedRequest: (sandbox, request, options, ref) =>
