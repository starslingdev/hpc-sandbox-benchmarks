...
export interface ComputeSdkDriverSpec<TCompute extends ComputeSdkLike> {
	...
	/**
	 * Post-allocation proof for request axes the create API cannot control or report up front.
	 * Returning unsupported rejects the request and makes the bridge tear the accepted handle down.
	 */
	readonly verifyCreatedRequest?: (
		sandbox: ComputeSdkSandboxOf<TCompute>,
		request: CreateRequest,
		options: DriverOperationOptions,
		ref: SandboxRef,
...
function validateComputeSdkCoverage(
	...
	if (needsRuntimeVerifier && !hasRuntimeVerifier) {
		invalidComputeSdkCoverage(
			provider,
			"a runtime-verified target axis requires verifyCreatedRequest",
		);
	}
...
async function verifyComputeSdkCreatedRequest<TCompute extends ComputeSdkLike>(
	provider: ProviderId,
	sandbox: ComputeSdkSandboxOf<TCompute>,
	request: CreateRequest,
	operationOptions: DriverOperationOptions | undefined,
	verify: NonNullable<ComputeSdkDriverSpec<TCompute>["verifyCreatedRequest"]>,
	sensitiveValues: readonly string[],
	ref: SandboxRef,
): Promise<void> {
	...
	try {
		...
		throw vendorContractFailure(
			provider,
			"created-request verification",
			"verifier returned an unreadable result",
			sensitiveValues,
			ref,
		);
	}
...
> {
	...
	const verifyCreatedRequest = options.verifyCreatedRequest;
	...
	validateComputeSdkCoverage(
		...
		verifyCreatedRequest !== undefined,
	);
	if (
	...
	return {
		async create(compute, request: CreateRequest, operationOptions?: DriverOperationOptions) {
			...
			try {
				...
				if (verifyCreatedRequest !== undefined) {
					await verifyComputeSdkCreatedRequest(
						provider,
						created,
						request,
						operationOptions,
						verifyCreatedRequest,
						sensitiveValues,
						ref,
					);
				}
...
function readRecoveryLocator<TCompute extends ComputeSdkLike>(
	...
	try {
		...
		throw vendorContractFailure(
			provider,
			"failed-create recovery locator",
			"recovery locator must be a readable nonempty name",
			sensitiveValues,
		);
	}
}
