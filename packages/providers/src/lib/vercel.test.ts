...
describe("Vercel ComputeSDK adapter", () => {
	...
	it("maps native filesystem methods used by detached polling", async () => {
		const calls: string[] = [];
		const fs = {
			readFile: async () => "done",
			...
			readdir: async () => [
				{ name: "done", isDirectory: () => false },
				{ name: "results", isDirectory: () => true },
			],
			lstat: async (path: string) => ({
				size: path.endsWith("done") ? 4 : 0,
				mtime: new Date("2026-07-31T01:00:00Z"),
			}),
			exists: async () => true,
			rm: async () => calls.push("remove"),
		};
		mockCreate(nativeSandbox({ fs }));
		const sandbox = await vercelCompute(config).sandbox.create();
		expect(await sandbox.filesystem.readFile("/tmp/done")).toBe("done");
		await sandbox.filesystem.writeFile("/tmp/done", "1");
		await sandbox.filesystem.mkdir("/tmp/results");
		expect(await sandbox.filesystem.exists("/tmp/done")).toBe(true);
		expect(await sandbox.filesystem.readdir("/tmp")).toEqual([
			{
				name: "done",
				type: "file",
				size: 4,
				modified: new Date("2026-07-31T01:00:00Z"),
			},
			{
				name: "results",
				type: "directory",
		...
		await sandbox.filesystem.remove("/tmp/results");
		expect(calls).toEqual(["write", "mkdir", "remove"]);
	});

	it("uses native detached execution and throws when launch transport fails", async () => {
		...
		mockCreate(nativeSandbox({ runCommand: async () => Promise.reject(new Error("offline")) }));
