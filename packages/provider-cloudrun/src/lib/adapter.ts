// The cloudrun ProviderAdapter. The gateway URL/secret must be passed as config — the
// @computesdk/cloud-run factory doesn't read its own env vars — so this package owns that slice
// and routes it through provider-core's validated env gate. Sandboxes execute inside the
// pre-deployed gateway service (remote mode), so there are no create-time spec knobs to pin:
// CPU/memory are the gateway's deploy-time flags (deploy it at the target spec for parity).
