...
const CPU_GENERATIONS: Record<string, CpuFingerprint> = {
	...
	"143/49": { name: "Zen 2 (Rome)", year: 2019 },
	"175/1": { name: "Zen 3 (Milan-X)", year: 2022 },
...
function resolveEpyc(brand: string): CpuFingerprint | undefined {
	...
	if (/\b7(R13|R32|763|B13)\b/.test(brand)) return { name: "Zen 3 (Milan)", year: 2021 };
	if (/\b7V73X\b/.test(brand)) return { name: "Zen 3 (Milan-X)", year: 2022 };
	if (/\b7V12\b/.test(brand)) return { name: "Zen 2 (Rome)", year: 2019 };
	if (/\b9R45\b/.test(brand)) return { name: "Zen 5 (Turin)", year: 2024 };
	if (/\b9R14\b/.test(brand)) return { name: "Zen 4 (Genoa)", year: 2022 };
	// Standard 4-digit EPYC numbering: the series digit and the final digit name the generation —
	// 9xx5 = Turin (Zen 5), 9xx4 = Genoa/Bergamo (Zen 4), 7xx3 = Milan (Zen 3), 7xx2 = Rome (Zen 2).
	const m = brand.match(/EPYC\s+(\d)\d{2}(\d)/);
	if (m) {
		const series = m[1];
		const gen = m[2];
...
function inferAmdFromIsa(brand: string, isa: CpuIsaHints): CpuFingerprint | undefined {
	...
	if (fam === 26) {
		// Family 0x1A = Zen 5. AVX-512 VP2INTERSECT is Zen 5-only on AMD.
		if (isa.hasAvx512Vp2intersect) {
