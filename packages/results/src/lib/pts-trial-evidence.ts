...
function provesSingleValueTrial(entry: PtsEntry): boolean {
	...
	try {
		const metadata: unknown = JSON.parse(entry.JSON);
		if (metadata === null || typeof metadata !== "object" || Array.isArray(metadata)) return false;
		if ("error" in metadata || !("test-run-times" in metadata)) return false;
		...
		provider.metrics
			...
			.map((metric) => {
				...
				for (const result of results) {
					...
					if (mapped.samples.length === 0) continue;
					if (
					...
					const source = entry?.RawString?.length ? "raw-string" : "aggregate-value";
					if (metric.ptsSampleSource !== undefined && metric.ptsSampleSource !== source)
						return evidence;
					if (entry?.RawString?.length)
						return { ...evidence, source: "raw-string" as const, observed: entry.RawString.length };
					return entry && provesSingleValueTrial(entry)
