...
export class StepRunner {
		...
		try {
			...
			for (;;) {
				...
				if (exitCode !== undefined) {
					const stdout = fs
						? await fs.readFile(logPath).catch(() => "")
	...
	private async pollDoneViaFs(
		...
		if (!(await fs.exists(donePath))) return undefined;
		return parseExitCode((await fs.readFile(donePath)).trim());
	}
	...
	private async pollDoneViaCat(donePath: string): Promise<number | undefined> {
		const probe = await this.sandbox.runCommand(
	...
	private async catFile(logPath: string): Promise<string> {
		const res = await this.sandbox
			.runCommand(`bash -c ${shellQuote(`cat ${logPath} 2>/dev/null || true`)}`)
			.catch(() => undefined);
		return res?.stdout ?? "";
	}
