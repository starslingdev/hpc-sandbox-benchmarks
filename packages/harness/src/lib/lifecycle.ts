...
/** Writes 64KiB of base64 to stdout — exec overhead including output streaming. */
const PAYLOAD_CMD = "head -c 65536 /dev/zero | base64";
...
export async function measureLifecycle(
	...
	const readinessMaxAttempts = Number.isFinite(rawAttempts)
		...
		: 40;
	const readinessRetryDelayMs = options.readinessRetryDelayMs ?? 250;
