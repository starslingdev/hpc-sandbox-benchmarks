...
describe("cliDriver", () => {
	...
	test("a create that never becomes ready fails at the request deadline", async () => {
		...
		expect((error.suppressed as Error).message).toMatch(/tama sandbox not ready within 25ms/);
		expect((error.error as DriverError).message).toMatch(/request deadline was exhausted/);
		expect(Date.now() - started).toBeLessThan(250);
		// The deadline does not restart for rollback. The rejected create retains ownership so
		// the process-level owner can perform the deferred cleanup with a fresh bounded attempt.
