...
## Task 7 — ENG-66 workflow↔registry sync test
- New drift gate `tooling/repo-checks/src/workflow-registry-sync.test.ts` (+ `lib/workflow-sync.ts`) ports THEIRS test/workflow-{env,suite}-sync.test.ts: parses `.github/workflows/bench-{smoke,matrix}.yml` via built-in `Bun.YAML.parse` (no new dep) and asserts the smoke `provider`/`suite` choice options == PROVIDERS ids / SUITE_NAMES, and every provider requiredEnvVar is wired into the "Run suite and normalize" step env of BOTH workflows with matching value expressions.
- **Workflow change (smallest slice to make the suite invariant real):** bench-smoke's `suite` input was free-text; made it a `type: choice` with `options: [cpu-node, system, memory, disk]` so adding a suite without updating the workflow now fails the gate (and constrains the dispatch input — defense-in-depth on top of the existing env passthrough).
- **Placement/deps:** lives where repo invariants live (tooling/repo-checks), which now declares `@sandbox-benchmarks/schema` as a devDep so boundary.test.ts stays green; runCheck() against the real files is the CI enforcement point, plus synthetic-drift unit coverage of every failure message.
- **Decision/insight:** bench-matrix has no provider/suite input (its matrix is computed from the registries by plan-matrix), so the choice-option invariant applies to bench-smoke only; the credential-env invariant covers both lanes. Set-equality (not order) is used since the smoke options list daytona-first for the default while PROVIDERS declares e2b-first.

## Task 8 — CI: actionlint + zizmor + persist-credentials hardening (no Linear issue)
- **New `.github/workflows/ci-lint.yml`** with two jobs — `actionlint` (workflow syntax/expressions/runner-label + embedded shellcheck) and `zizmor` (`--persona regular --min-severity medium --min-confidence high`) over `.github/workflows/`. Both run on the self-hosted `starsling-ubuntu-24.04-2` (ubuntu-latest produces no runs here) behind the same fork-PR guard as ci.yml, path-filtered to `.github/**` + `mise.toml`.
- **Tooling via mise, not curl/pipx (DRY + framework-native):** added `aqua:rhysd/actionlint@1.7.12` and `aqua:zizmorcore/zizmor@1.26.1` to mise.toml — aqua does checksum + attestation verification (the task's "sha256-verified binary", same posture as typos/shellcheck/hadolint), and the runner already wires mise-action. Added `.github/actionlint.yaml` registering the self-hosted runner label so actionlint stops flagging every `runs-on:`.
- **persist-credentials: false** on every `actions/checkout` EXCEPT bench-matrix.yml's `publish` job, which commits + `git push`es the promoted dataset (it needs the token; documented inline + in `.github/zizmor.yml`). Fixed one SC2155 in toolchain-image.yml (`export BUILD_DATE=$(…)` → declare then export) that actionlint's embedded shellcheck flagged.
- **TDD drift gate** `tooling/repo-checks/src/workflow-hardening.{ts,test.ts}`: parses every workflow (Bun.YAML) and asserts the persist-credentials posture (with a rot-proof pushing-checkout allowlist) + that ci-lint runs actionlint/zizmor at the un-loosened threshold. Verified locally: actionlint clean, zizmor clean at the gate (1 Low-confidence artipacked accepted via zizmor.yml), full gate green.

## Task 9 — ENG-62 provider-join runtime assert + CLI discovery flags
- **Runtime join guard** `packages/providers/src/lib/join.ts` (`assertProviderJoin`) called at module load in `index.ts` alongside the existing compile-time `Record<ProviderId, …>` typing: a one-sided provider (schema entry without an adapter, or vice versa) now throws naming the exact offenders, catching cross-version/published-build drift the type-checker never saw. Unit-tested both one-sided directions + happy path.
- **Agent-friendly discovery** in `apps/cli/src/lib/discovery.ts` (`handleDiscovery(argv, HELP)` + provider/suite listings read straight from the schema registries) wired into `plan-matrix`, `bench-suite`, `leaderboard`: `--help`, `--list-providers`, `--list-suites`, `--json`. Positional args preserved (bins slice argv and fall through when no discovery flag).
- **Decision/insight:** centralised the flag dispatch in one shared helper (cross-CLI vocabulary consistency, unit-testable without spawning) rather than per-bin; kept `plan-matrix` bare = single-line matrix JSON (the $GITHUB_OUTPUT contract) and `bench-suite` bare = daytona/cpu-node local default untouched so CI invocations don't change. Listings render JSON under `--json`, human TAB-lines otherwise.

## Task 10 — ENG-69 ADRs + catalog/analysis assertions
- **New `docs/adr/`** (index + 5 Context/Decision/Consequences records), linked from the README docs banner: arktype parse-don't-validate boundary; enforced dependency DAG; generated PTS catalog + drift gate; raw-first consumption-layer candidate→promote; host-vs-effective spec split. Ported the ADR format/spirit from THEIRS `docs/adr/` (esp. 0006 raw-history-normalize) without copying its web/SPA decisions.
- **Catalog assertions** (`catalog.test.ts`): every `METRIC_CATALOG` id matches `/^[a-z0-9]+(_[a-z0-9]+)*$/` (snake_case slug — ids are stability-contract keys in URLs/filenames), and no `derived` metric carries a `pts` source (the two provenances are mutually exclusive).
- **`percentileOf(samples, p)`** exported from `analysis.ts`: public entry to the same R-7 helper `aggregate` uses (sorts a copy, defers to private `percentile`), so a one-off percentile can't diverge from the Aggregates distribution. Same fail-fast posture (empty/non-finite/p∉[0,1]; `!(p>=0&&p<=1)` rejects NaN too); unit-tested incl. parity with `aggregate`.
- **Decision/insight:** assertions were pre-satisfied by current data, so they're regression guards (TDD: catalog tests green on first run, `percentileOf` test red until the export landed). Drift gate untouched — no PTS-catalog content changed.

## Task 11 — ENG-67 publish real Daytona run → dataset + LEADERBOARD.md
- **Promoted the REAL captured run** (`data/runs/zen5-collected-1782365479.json`, `sha=zen5-ssh`) through the real pipeline: `aggregate.ts` (single shard → candidate in scratch) then `promote.ts` → `data/dataset/runs/<id>.json` + `data/dataset/index.json`. Nothing fabricated; aggregate enriched daytona with the derived `usd_per_hour` economics metric (2 metrics total).
- **promote gate passed** on 1 validated provider (daytona; e2b/modal stay `pending` with empty metrics), so it published rather than refusing. Rendered `LEADERBOARD.md` from the published dataset via `leaderboard.ts` — cpu (Node.js web tooling 22.77 runs/s) + economics ($0.1494/hr) tables.
- **Single-provider snapshot:** only Daytona is validated, so each leaderboard table has one ranked row — honest, not padded with placeholder rivals. `data/raw`+`data/runs` stay gitignored/transient; committed `data/dataset/` JSON is biome-clean (lint:fix touched the run doc).
- **Decision:** used scratch as the transient candidate dir (CI uses `data/candidate`) since only `data/dataset/` should be committed. Green gate (typecheck/test/lint/spell) all pass.
