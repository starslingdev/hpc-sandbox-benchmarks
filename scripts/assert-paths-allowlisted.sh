...
#   a single-level extension-pinned glob   docs/figures/*.svg
#
# The glob form exists because the leaderboard publishes one SVG chart per realworld suite, and that
...
# directory, and a fixed extension. `docs/figures/*.svg` cannot match `docs/figures/nested/x.svg`,
...
for path in "$@"; do
  ...
  case "$path" in
    */'*.'*)
      # The DIRECTORY must be literal — `docs/**/*.svg` gets this far otherwise, and `**` is exactly
