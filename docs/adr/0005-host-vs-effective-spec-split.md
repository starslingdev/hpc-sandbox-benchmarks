...
and **never reflects the effective spec**. The aggregate path sets `hostHeterogeneous` when a
provider's merged shards disclosed differing host CPUs/microarchs, surfacing a scheduling confound the
...
- Heterogeneous scheduling is disclosed, not hidden: `hostHeterogeneous` flags a provider whose shards
  didn't all land on the same hardware, so the comparison stays honest about that confound.
